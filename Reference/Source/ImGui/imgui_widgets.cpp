...
bool ImGui::StealthButton(const char* str_id, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    // Cannot use zero-size for StealthButton(). Unlike Button() there is not way to fallback using the label size.
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        StealthButton("hsv", ImVec2(sv_picker_size + style.ItemInnerSpacing.x + bars_width, sv_picker_size));
        if (IsItemActive())
        {
    ...
    {
        // SV rectangle logic
        StealthButton("sv", ImVec2(sv_picker_size, sv_picker_size));
        if (IsItemActive())
        {
        ...
        StealthButton("hue", ImVec2(bars_width, sv_picker_size));
        if (IsItemActive())
        {
    ...
    if (alpha_bar)
    {
        ...
        StealthButton("alpha", ImVec2(bars_width, sv_picker_size));
        if (IsItemActive())
        {
...
void ImGui::ColorPickerOptionsPopup(const float* ref_col, ImGuiColorEditFlags flags)
{
    ...
    if (allow_opt_picker)
    {
        ...
        {
            // Draw small/thumbnail version of each picker type (over an Stealth button for selection)
