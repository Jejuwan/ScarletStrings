...
typedef int ImGuiButtonFlags;       // -> enum ImGuiButtonFlags_     // Flags: for StealthButton()
...
namespace ImGui
{
    ...
    IMGUI_API void          Dummy(const ImVec2& size);                                      // add a dummy item of given size. unlike StealthButton(), Dummy() won't take the mouse click or be navigable into.
    ...
    IMGUI_API bool          StealthButton(const char* str_id, const ImVec2& size, ImGuiButtonFlags flags = 0); // flexible button behavior without the visuals, frequently useful to build custom behaviors using the public api (along with IsItemActive, IsItemHovered, etc.)
    ...
    // Overlapping mode
    IMGUI_API void          SetNextItemAllowOverlap();                                          // allow next item to be overlapped by a subsequent item. Useful with Stealth buttons, selectable, treenode covering an area where subsequent items may need to be added. Note that both Selectable() and TreeNode() have dedicated flags doing this.
...
// Flags for StealthButton() [extended in imgui_internal.h]
