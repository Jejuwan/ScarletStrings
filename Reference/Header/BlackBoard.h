...
public:
	...
	HRESULT												Add_WeakSystem(const wstring& wstrKey, weak_ptr<class ISystem> pSystem);
	...
	template<typename T>
	shared_ptr<T>										Get_WeakSystem(const wstring& wstrKey);
	shared_ptr<class ISystem>							Get_WeakSystem(const wstring& wstrKey);
...
private:
	...
	unordered_map<wstring, weak_ptr<class ISystem>>		m_umapWeakSystem;
