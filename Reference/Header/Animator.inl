...
template<typename T>
inline _float CAnimator::Get_TrackPosition(T eAnimation) const
{
	return Get_TrackPosition(IDX(eAnimation));
}
...
template<typename T>
inline void CAnimator::Reset_Animation(T _eAnimation)
{
	Reset_Animation(IDX(_eAnimation));
}
