...
public:
	void Initialize(const ComPtr<ID3D11Device>&, const ComPtr<ID3D11DeviceContext>&,
		...
		const _int iWidth, const _int iHeight, const _int iBloomLevels, const IMAGE_PROCESS eType);
	void Tick(_float fTimeDelta);
	HRESULT Render() { return E_FAIL; }
	HRESULT Render(IMAGE_PROCESS, _uint iFilterPass, function<HRESULT(shared_ptr<class CShader>)> = nullptr);
	HRESULT Render(IMAGE_PROCESS, _uint iFilterPass);
	HRESULT Render(IMAGE_PROCESS, _uint iFilterPass, _uint iLUTIndex);

public:
	void FadeInExposure(_float fEndExposure, _float fTimeScale);
	void FadeInGamma(_float fEndGamma, _float fTimeScale);
	void FadeOutExposure(_float fEndExposure, _float fTimeScale);
	void FadeOutGamma(_float fEndGamma, _float fTimeScale);

public:
	void Set_Exposure(_float fExposure);
	void Set_Gamma(_float fGamma);
...
public:
	void MaskingLUT(_bool bMasking, shared_ptr<class CTexture>);
