...
public:
#ifdef _DEBUG
	const _uint							Get_NumPasses() const	{ return m_iNumPasses; }
#endif

	HRESULT								BeginPass(const _uint iPassIndex, _bool bCompute = false);
	...
	HRESULT								Bind_UnorderedAccessView(const _char* szConstantName, _In_ ComPtr<ID3D11UnorderedAccessView>);
	HRESULT								Bind_UnorderedAccessViews(const _char* szConstantName, _In_ ID3D11UnorderedAccessView**, _uint iOffset = 0, _uint iCount = 1);

	HRESULT								Dispatch(_uint iPass, _uint iX, _uint iY, _uint iZ);
