...
public:
	...
	void Render();
	void Clear(_float4 vClearColor);

	void SetShaderResources(const std::vector<ComPtr<ID3D11ShaderResourceView>>& vecResources);
	void SetRenderTargets(const std::vector<ComPtr<ID3D11RenderTargetView>>& vecTargets);

public:
	void Bind_RawValue(const char* pConstantName, const void* pData, _uint iByteSize);

public:
	void Append_LUTTexture(const _wchar* pLUTTag, shared_ptr<class CTexture>);
	void Bind_LUTTexture(_uint iLUTFilterIndex);
