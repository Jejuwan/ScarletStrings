...
void CPostprocess::Tick(_float fTimeDelta)
{
	if (m_bFadeInExposure)
	{
		...
		{
			m_fExposure += fTimeDelta * fTimeScale;
		}
		else
		{
	...
	if (m_bFadeInGamma)
	{
		...
		{
			m_fGamma += fTimeDelta * fTimeScale;
		}
		else
		{
			m_fGamma = fEndGamma;
...
HRESULT CPostprocess::Render(IMAGE_PROCESS eProcessType, _uint iFilterPass, function<HRESULT(shared_ptr<CShader>)> _fpListener)
{
...
HRESULT CPostprocess::Render(IMAGE_PROCESS eProcessType, _uint iFilterPass, _uint iLUTIndex)
{
	m_pCombineFilter->Bind_LUTTexture(iLUTIndex);

	return Render(eProcessType, iFilterPass, nullptr);
}

void CPostprocess::FadeInExposure(_float fEndExposure, _float fTimeScale)
{
	m_bFadeInExposure = true;

	m_FadeInExposurePair.first = fEndExposure;
	m_FadeInExposurePair.second = fTimeScale;
}

void CPostprocess::FadeInGamma(_float fEndGamma, _float fTimeScale)
{
	m_bFadeInGamma = true;

	m_FadeInGammaPair.first = fEndGamma;
	m_FadeInGammaPair.second = fTimeScale;
}

void CPostprocess::FadeOutExposure(_float fEndExposure, _float fTimeScale)
{
	m_bFadeOutExposure = true;

	m_FadeOutExposurePair.first = fEndExposure;
	m_FadeOutExposurePair.second = fTimeScale;
}

void CPostprocess::FadeOutGamma(_float fEndGamma, _float fTimeScale)
{
	m_bFadeOutGamma = true;

	m_FadeOutGammaPair.first = fEndGamma;
	m_FadeOutGammaPair.second = fTimeScale;
}

void CPostprocess::Set_Exposure(_float fExposure)
{
...
void CPostprocess::Set_Gamma(_float fGamma)
{
	m_fGamma = fGamma;
...
void CPostprocess::Set_BloomStrength(_float fStrength)
{
	m_fBloomStrength = fStrength;
...
void CPostprocess::MaskingLUT(_bool bMasking, shared_ptr<class CTexture> pMaskingTexture)
{
	m_pCombineFilter->MaskingLUT(bMasking, pMaskingTexture);
}
