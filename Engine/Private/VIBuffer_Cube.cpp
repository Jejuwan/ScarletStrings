...
#pragma endregion
	...
	{
		PxVertexBuffer[i] = { pVertices[i].vPosition.x,pVertices[i].vPosition.y,pVertices[i].vPosition.z };
	}

	shared_ptr<PxU32[]> PxIndexBuffer(new PxU32[m_iNumIndices]);
	for (_uint i = 0; i < m_iNumIndices; i++)
		PxIndexBuffer[i] = pIndices[i];
	...
	meshDesc.triangles.count = m_iNumIndices;
