...
{
	m_umapMultiRenderTarget.clear();
	m_umapMultiDepthStencil.clear();
	m_umapRenderTarget.clear();
	m_umapDepthStencil.clear();

	if (!m_bOriginalTarget)
	{
		End_MultiRenderTarget();
	}

	m_pBackBufferView->AddRef();
...
ComPtr<ID3D11Texture2D> CRenderTarget_Manager::Get_Texture2D(const wstring& wstrTargetTag)
{
	if (!Exist_RenderTarget(wstrTargetTag))
	{
		MSG_RETURN(nullptr, "CRenderTarget_Manager::Get_Texture2D", "Invalid RenderTarget");
	}

	return m_umapRenderTarget[wstrTargetTag]->Get_Texture2D();
}
...
ComPtr<ID3D11RenderTargetView> CRenderTarget_Manager::Get_RenderTargetView(const wstring& wstrTargetTag)
{
	if (!Exist_RenderTarget(wstrTargetTag))
	{
		MSG_RETURN(nullptr, "CRenderTarget_Manager::Get_RenderTargetView", "Invalid RenderTarget");
	}

	return m_umapRenderTarget[wstrTargetTag]->Get_RenderTargetView();
}
...
HRESULT CRenderTarget_Manager::Begin_MultiRenderTaget(const wstring& _wstrMultiTargetTag, _uint _iCopyResource, initializer_list<_uint> _il)
{
	if (!Exist_MultiRenderTarget(_wstrMultiTargetTag))
	{
	...
	{
		if (_il.end() == std::find(_il.begin(), _il.end(), iIndex))
		{
...
HRESULT CRenderTarget_Manager::Copy_From_BackBuffer(const wstring& _wstrTargetTag)
{
	if (!Exist_RenderTarget(_wstrTargetTag))
	{
		MSG_RETURN(E_FAIL, "CRenderTarget_Manager::Copy_From_BackBuffer", "Invalid RenderTarget");
	}

	ComPtr<ID3D11Resource> pSrc, pDst;
	m_pBackBufferView->GetResource(pSrc.GetAddressOf());
	m_umapRenderTarget[_wstrTargetTag]->Get_RenderTargetView()->GetResource(pDst.GetAddressOf());
	m_pContext->CopyResource(pDst.Get(), pSrc.Get());

	return S_OK;
}

HRESULT CRenderTarget_Manager::Copy_To_BackBuffer(const wstring& _wstrTargetTag)
{
	if (!Exist_RenderTarget(_wstrTargetTag))
	{
		MSG_RETURN(E_FAIL, "CRenderTarget_Manager::Copy_To_BackBuffer", "Invalid RenderTarget");
	}
...
HRESULT CRenderTarget_Manager::Copy_RenderTarget(const wstring& wstrDestTargetTag, const wstring& wstrSrcTargetTag)
{
	if (!Exist_RenderTarget(wstrDestTargetTag) || !Exist_RenderTarget(wstrSrcTargetTag))
	{
		MSG_RETURN(E_FAIL, "CRenderTarget_Manager::Copy_RenderTarget", "Invalid RenderTarget");
	}

	ComPtr<ID3D11Resource> pSrc, pDst;
	m_umapRenderTarget[wstrDestTargetTag]->Get_RenderTargetView()->GetResource(pDst.GetAddressOf());
	m_umapRenderTarget[wstrSrcTargetTag]->Get_RenderTargetView()->GetResource(pSrc.GetAddressOf());
	m_pContext->CopyResource(pDst.Get(), pSrc.Get());
