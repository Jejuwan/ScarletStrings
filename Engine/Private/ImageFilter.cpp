...
void CImageFilter::Clear(_float4 vClearColor)
{
	for (auto pRTV : m_vecRenderTargets)
		m_pContext->ClearRenderTargetView(pRTV.Get(), &vClearColor.x);
}

void CImageFilter::SetShaderResources(const std::vector<ComPtr<ID3D11ShaderResourceView>>& vecResources)
{
	m_vecShaderResources.clear();
	for (const auto pResource : vecResources) {
		m_vecShaderResources.push_back(pResource);
	}
}

void CImageFilter::SetRenderTargets(const std::vector<ComPtr<ID3D11RenderTargetView>>& vecTargets)
{
	m_vecRenderTargets.clear();
	for (const auto pTarget : vecTargets) {
		m_vecRenderTargets.push_back(pTarget);
	}
}

void CImageFilter::Bind_RawValue(const char* pConstantName, const void* pData, _uint iByteSize)
{
	m_pPixelShader->Bind_RawValue(pConstantName, pData, iByteSize);
}

void CImageFilter::Bind_LUTTexture(_uint iLUTFilterIndex)
{
	m_bExecuteLUT = true;
	m_iLUTFilterIndex = iLUTFilterIndex;
}

void CImageFilter::MaskingLUT(_bool bMasking, shared_ptr<class CTexture> pMaskingTexture)
{
	m_bMaskingLUT = bMasking;
	if (m_bMaskingLUT)
	{
		m_pMaskingTexture = pMaskingTexture;
	}
	else
	{
		m_pMaskingTexture = pMaskingTexture;
...
void CImageFilter::Append_LUTTexture(const _wchar* pLUTTag, shared_ptr<class CTexture> _pLUTTexture)
{
	m_vecLUTFilters.push_back({ pLUTTag, _pLUTTexture });
}
