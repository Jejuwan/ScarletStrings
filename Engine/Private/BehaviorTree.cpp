...
void CBehaviorTree::Tick(_float _fTimeDelta)
{
	...
	m_pRootNode->Tick(_fTimeDelta);

	m_pRootNode->Invoke(_fTimeDelta);
}

void CBehaviorTree::Late_Tick(_float _fTimeDelta)
{
	m_pRootNode->Late_Tick(_fTimeDelta);
}
...
HRESULT CBehaviorTree::Set_RootNode(shared_ptr<CNode> _pNode)
{
	...
	m_pRootNode = _pNode;
#ifdef _DEBUG
	m_pRootNode->Set_RootNode();
	m_pRootNode->Refresh_Node(m_pRootNode);
#endif
