...
#endif
//	iHLSLFlag |= D3DCOMPILE_PACK_MATRIX_ROW_MAJOR;
...
HRESULT CShader::BeginPass(const _uint _iPassIndex, _bool bCompute)
{
...
HRESULT CShader::Bind_ShaderResourceViews(const _char* _szConstantName, _In_ ID3D11ShaderResourceView** _ppShaderResourceViews, _uint _iOffset, _uint _iCount)
{
	if (0 == _iCount)
	{
		return S_FALSE;
	...
	if (FAILED(pShaderResourceVariable->SetResourceArray(_ppShaderResourceViews, _iOffset, _iCount)))
	{
...
HRESULT CShader::Bind_UnorderedAccessView(const _char* _szConstantName, _In_ ComPtr<ID3D11UnorderedAccessView> _pUnorderedAccessView)
{
	if (nullptr == m_pEffect)
	{
		return E_FAIL;
	...
	ID3DX11EffectUnorderedAccessViewVariable* pUnorderedAccessVariable = m_pEffect->GetVariableByName(_szConstantName)->AsUnorderedAccessView();
	if (nullptr == pUnorderedAccessVariable)
	{
		MSG_RETURN(E_FAIL, "CShader::Bind_UnorderedAccessView", "Failed to GetVariableByName");
	}

	if (FAILED(pUnorderedAccessVariable->SetUnorderedAccessView(_pUnorderedAccessView.Get())))
	{
		MSG_RETURN(E_FAIL, "CShader::Bind_UnorderedAccessView", "Failed to SetUnorderedAccessView");
	}

	return S_OK;
}

HRESULT CShader::Bind_UnorderedAccessViews(const _char* _szConstantName, _In_ ID3D11UnorderedAccessView** _ppUnorderedAccessViews, _uint _iOffset, _uint _iCount)
{
	if (0 == _iCount)
	{
		return S_FALSE;
	...
	if (nullptr == pUnorderedAccessVariable)
	{
		MSG_RETURN(E_FAIL, "CShader::Bind_UnorderedAccessViews", "Failed to GetVariableByName");
	}

	if (FAILED(pUnorderedAccessVariable->SetUnorderedAccessViewArray(_ppUnorderedAccessViews, _iOffset, _iCount)))
	{
		MSG_RETURN(E_FAIL, "CShader::Bind_UnorderedAccessViews", "Failed to SetResource");
	}

	return S_OK;
}

HRESULT CShader::Dispatch(_uint _iPass, _uint _iX, _uint _iY, _uint _iZ)
{
	if (FAILED(BeginPass(_iPass, true)))
	{
		MSG_RETURN(E_FAIL, "CShader::Dispatch", "Failed to BeginPass");
	}

	m_pContext->Dispatch(_iX, _iY, _iZ);
