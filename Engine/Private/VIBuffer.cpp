...
void CVIBuffer::Copy_Vertices(void* _pDest)
{
	memcpy(_pDest, m_pVertices.get(), sizeof(_float3) * m_iNumVertices);
}

void CVIBuffer::Copy_Indices(void* _pDest)
{
	memcpy(_pDest, m_pIndices.get(), sizeof(_uint) * m_iNumIndices);
}
