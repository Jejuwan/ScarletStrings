...
HRESULT CCamera::Initialize(any _arg)
{
	...
	pPipeLine->Set_OrthoProjection(XMMatrixOrthographicLH(m_tCameraDesc.fWidth / m_tCameraDesc.fHeight, 1.f, m_tCameraDesc.fNear, m_tCameraDesc.fFar));
...
HRESULT CCamera::Render()
{
	...
	{
	case TYPE::PERSPECTIVE:
		pPipeLine->Set_Transform(PIPELINE::WORLD,		m_pTransform->Get_Matrix());
		pPipeLine->Set_Transform(PIPELINE::VIEW,		m_pTransform->Get_Matrix().inverse());
		pPipeLine->Set_Transform(PIPELINE::PROJECTION,	m_mProjection);
		break;

	case TYPE::ORTHOGONAL:
		pPipeLine->Set_Transform(PIPELINE::WORLD,		m_pTransform->Get_Matrix());
		pPipeLine->Set_Transform(PIPELINE::VIEW,		m_mView);
		pPipeLine->Set_Transform(PIPELINE::PROJECTION,	m_mProjection);
		break;
	}
...
void CCamera::Set_State(TRANSFORM eState, _vector vState)
{
	m_pTransform->Set_State(eState, vState);
}

_vector CCamera::Get_State(TRANSFORM eState)
{
	return m_pTransform->Get_State(eState);
}

void CCamera::Lock(_bool bLock)
{
