...
HRESULT CBlackBoard::Add_WeakSystem(const wstring& _wstrKey, weak_ptr<ISystem> _pSystem)
{
	...
	m_umapWeakSystem.emplace(_wstrKey, _pSystem);
...
shared_ptr<ISystem> CBlackBoard::Get_WeakSystem(const wstring& _wstrKey)
{
	if (m_umapWeakSystem.end() != m_umapWeakSystem.find(_wstrKey))
	{
		return m_umapWeakSystem[_wstrKey].lock();
	}
