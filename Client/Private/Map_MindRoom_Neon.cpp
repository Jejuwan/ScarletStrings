...
CMap_MindRoom_Neon::CMap_MindRoom_Neon(ComPtr<ID3D11Device> _pDevice, ComPtr<ID3D11DeviceContext> _pContext)
	: CGameObject(_pDevice, _pContext)
{
}

CMap_MindRoom_Neon::CMap_MindRoom_Neon(const CMap_MindRoom_Neon& _rhs)
	:CGameObject(_rhs)
	, m_iIndex(_rhs.m_iIndex)
{
}

HRESULT CMap_MindRoom_Neon::Initialize_Prototype(_uint iIndex)
{
...
HRESULT CMap_MindRoom_Neon::Initialize(any pArg)
{
	if (FAILED(__super::Initialize()))
	{
		MSG_RETURN(E_FAIL, "CMap_MindRoom_Neon::Initialize", "Failed to CGameObject::Initialize");
	}
	...
	// Hide Mesh
	{
		...
		if (m_wstrModelTag == PROTOTYPE_COMPONENT_MODEL_MINDROOM_TENKYU)
		{
			m_tInstModelDesc.iPassIndex = 5;
		}
		if (m_wstrModelTag == PROTOTYPE_COMPONENT_MODEL_MINDROOM_FLOOR)
		{
			m_tInstModelDesc.iPassIndex = 21;
			m_tInstModelDesc.fpBindResource = [&](shared_ptr<CModel>, shared_ptr<CShader> pShader)
				{
					...
					if (FAILED(pShader->Bind_Float("g_fSampleRadius", 5.f)))
					{
		...
		if (m_wstrModelTag == PROTOTYPE_COMPONENT_MODEL_MINDROOM_TWIST)
		{
			m_tInstModelDesc.iPassIndex = 5;
		}
		if (m_wstrModelTag == PROTOTYPE_COMPONENT_MODEL_MINDROOM_REDLINE)
		{
			m_tInstModelDesc.iPassIndex = 5;
		}
	}

	if (Is_Twist(m_strName))
	{
		std::random_device rd;
		std::mt19937 gen(rd());
		std::uniform_real_distribution<_float> RandomNumber(0.25f, 0.75f);
...
void CMap_MindRoom_Neon::Tick(_float fTimeDelta)
{
	if (m_bDead)
		return;
	...
	if (Is_Twist(m_strName))
	{
		m_pTransform->Rotate_Lerp(TRANSFORM::UP, fTimeDelta * m_fSpeed);
	}
}

void CMap_MindRoom_Neon::Late_Tick(_float fTimeDelta)
{
	if (m_bDead)
		return;
...
HRESULT CMap_MindRoom_Neon::Render()
{
	return S_OK;
}

HRESULT CMap_MindRoom_Neon::Render_ShadowDepth(shared_ptr<CLight> _pShadow)
{
	return S_OK;
}

HRESULT CMap_MindRoom_Neon::Render_OutLine()
{
	return S_OK;
}

HRESULT CMap_MindRoom_Neon::Render_Neon()
{
	return S_OK;
}

void CMap_MindRoom_Neon::Set_NeonColor(const _float4& vNeonColor)
{
	m_tMaterialDesc.vDiffuse = vNeonColor;
	m_tMaterialDesc.vAmbient = vNeonColor;
	
	m_pModelRenderer->Set_MaterialDesc(m_wstrModelTag, &m_tMaterialDesc);
}

HRESULT CMap_MindRoom_Neon::Ready_Components()
{
...
void CMap_MindRoom_Neon::Using_Map_Tool()
{
...
_bool CMap_MindRoom_Neon::Is_Twist(wstring strTwist)
{
	_bool	bCheck = { false };

	std::wregex pattern(L"_number\\d+");
	wstring TempStr = regex_replace(strTwist, pattern, L"");

	if (TempStr == TEXT("MindRoom_Twist"))
	{
		bCheck = true;
	}
...
void CMap_MindRoom_Neon::Add_ModelCom()
{
	if (m_iIndex == 0)
	{
		m_wstrModelTag = PROTOTYPE_COMPONENT_MODEL_MINDROOM_TENKYU;
	}
	if (m_iIndex == 1)
	{
		m_wstrModelTag = PROTOTYPE_COMPONENT_MODEL_MINDROOM_FLOOR;
	}
	if (m_iIndex == 2)
	{
		m_wstrModelTag = PROTOTYPE_COMPONENT_MODEL_MINDROOM_TWIST;
	}	
	if (m_iIndex == 3)
	{
		m_wstrModelTag = PROTOTYPE_COMPONENT_MODEL_MINDROOM_REDLINE;
	}

	m_pTransform = m_pModelRenderer->Add_Instance(m_wstrModelTag);
}

shared_ptr<CMap_MindRoom_Neon> CMap_MindRoom_Neon::Create(ComPtr<ID3D11Device> _pDevice, ComPtr<ID3D11DeviceContext> _pContext, _int iIndex)
{
	shared_ptr<CMap_MindRoom_Neon> pInstance = make_private_shared(CMap_MindRoom_Neon, _pDevice, _pContext);

	if (FAILED(pInstance->Initialize_Prototype(iIndex)))
	{
		MSG_RETURN(nullptr, "CMap_MindRoom_Neon::Create", "Failed to Initialize_Prototype");
	}

	return pInstance;
}

shared_ptr<CGameObject> CMap_MindRoom_Neon::Clone(any pArg)
{
	shared_ptr<CMap_MindRoom_Neon> pInstance = make_private_shared_copy(CMap_MindRoom_Neon, *this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_RETURN(nullptr, "CMap_MindRoom_Neon::Clone", "Failed to Initialize");
	}
