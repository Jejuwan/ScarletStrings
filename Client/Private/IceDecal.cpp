...
CIceDecal::CIceDecal(ComPtr<ID3D11Device> pDevice, ComPtr<ID3D11DeviceContext> pContext)
	: CDecal(pDevice, pContext)
{
}

CIceDecal::CIceDecal(const CDecal& rhs)
	: CDecal(rhs)
{
}
...
HRESULT CIceDecal::Initialize(any tIceDecalDesc)
{
...
void CIceDecal::Tick(_float fTimeDelta)
{
	if (m_bFetched)
	{
		__super::Tick(fTimeDelta);

		m_fAccTime += fTimeDelta * m_fTimeScale;
...
void CIceDecal::Late_Tick(_float fTimeDelta)
{
	if (m_bFetched)
	{
...
HRESULT CIceDecal::Render()
{
	...
	{
		MSG_RETURN(E_FAIL, "CDecal::Render", "CDecal::Render");
	}
...
HRESULT CIceDecal::Fetch(any tIceDecalDesc)
{
	ICEDECALDESC tIceDesc = any_cast<ICEDECALDESC>(tIceDecalDesc);
...
shared_ptr<CIceDecal> CIceDecal::Create(ComPtr<ID3D11Device> pDevice, ComPtr<ID3D11DeviceContext> pContext)
{
	shared_ptr<CIceDecal> pInstance = make_private_shared(CIceDecal, pDevice, pContext);
...
shared_ptr<CGameObject> CIceDecal::Clone(any tIceDecalDesc)
{
	shared_ptr<CIceDecal> pInstance = make_private_shared_copy(CIceDecal, *this);

	if (FAILED(pInstance->Initialize(tIceDecalDesc)))
	{
