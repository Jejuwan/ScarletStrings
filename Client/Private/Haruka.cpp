...
CHaruka::CHaruka(ComPtr<ID3D11Device> _pDevice, ComPtr<ID3D11DeviceContext> _pContext)
	: CCharacter(_pDevice, _pContext)
{
}

CHaruka::CHaruka(const CHaruka& _rhs)
	: CCharacter(_rhs)
{
}
...
HRESULT CHaruka::Initialize_Prototype()
{
	...
	m_umapComponentArg[COMPONENT::MODEL] = make_pair(PROTOTYPE_COMPONENT_MODEL_HARUKA, g_aNull);

	return S_OK;
}

HRESULT CHaruka::Initialize(any)
{
	...
	m_pTransform->Set_State(TRANSFORM::POSITION, _float4(-0.5f, 0.f, 0.5f, 1.f));
...
void CHaruka::Tick(_float fTimeDelta)
{
	...
	{
	if (ImGui::BeginMenu("Haruka"))
	{
	if (ImGui::BeginMenu("Animation Settings"))
	{
	ImGui::Text("Haruka Animation Index : %d", m_iCurrentAnimIndex);

	ImGui::SameLine();
	if (ImGui::ArrowButton("Haruka Left", ImGuiDir_Left))
	{
		--m_iCurrentAnimIndex;
		if (m_iCurrentAnimIndex < 0)
			m_iCurrentAnimIndex = _int(ANIMATION::HARUKA::MAX) - 1;

		m_pModel->Set_Animation(m_iCurrentAnimIndex);
	}
	ImGui::SameLine();
	if (ImGui::ArrowButton("Haruka Right", ImGuiDir_Right))
	{
		m_iCurrentAnimIndex++;
		if (m_iCurrentAnimIndex >= _int(ANIMATION::HARUKA::MAX))
	...
	if (ImGui::BeginMenu("Tramsform Batcher"))
	{
		...
		{
			ImGui::SeparatorText("Haruka Transform");
			vNewPosition = vTranslation;
			if (ImGui::DragFloat3("Haruka Position", &vNewPosition.x, 0.005f, -FLT_MAX, +FLT_MAX))
				bTransformDirtyFlag = true;
			vNewRotation = Function::QuaternionToEuler(vQuaternion, false);
			if (ImGui::DragFloat3("Haruka Rotation", &vNewRotation.x, 0.5f, -FLT_MAX, +FLT_MAX))
				bTransformDirtyFlag = true;
		}
	...
	if (ImGui::CollapsingHeader("Haruka Material"))
	{
...
void CHaruka::Late_Tick(_float fTimeDelta)
{
...
HRESULT CHaruka::Render()
{
	...
	{
		MSG_RETURN(E_FAIL, "CHaruka::Render", "Failed to CGameObject::Bind_Transform");
	}
	...
	for (_uint i = 0; i < m_vecMeshPassIndexPair.size(); ++i)
	{
		...
		{
			MSG_RETURN(E_FAIL, "CHaruka::Render", "Failed to Bind_ShaderResourceViews");
		}
		if (FAILED(m_pModel->Bind_ShaderOptions(iMeshIndex, m_pShader)))
		{
			MSG_RETURN(E_FAIL, "CHaruka::Render", "Failed to Bind_ShaderOptions");
		}
		if (FAILED(m_pModel->Bind_BoneMatrices(iMeshIndex, m_pShader, SHADER_BONE)))
		{
			MSG_RETURN(E_FAIL, "CHaruka::Render", "Failed to Bind_BoneMatrices");
		}

		if (FAILED(m_pModel->Render(iMeshIndex, m_pShader, iPassIndex)))
		{
			MSG_RETURN(E_FAIL, "CHaruka::Render", "Failed to Render Model");
		}
	}
...
void CHaruka::Move_Mouth()
{
	m_pModel->Set_Animation(1);
}

void CHaruka::Stop_Mouth()
{
}

HRESULT CHaruka::Render_OutLine()
{
	return S_OK;
}

HRESULT CHaruka::Ready_Components()
{
	if (FAILED(__super::Ready_Components()))
	{
		MSG_RETURN(E_FAIL, "CHaruka::Ready_Components", "Failed to CGameObject::Ready_Components");
	}

	return S_OK;
}

shared_ptr<CHaruka> CHaruka::Create(ComPtr<ID3D11Device> _pDevice, ComPtr<ID3D11DeviceContext> _pContext)
{
	shared_ptr<CHaruka> pInstance = make_private_shared(CHaruka, _pDevice, _pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_RETURN(nullptr, "CHaruka::Create", "Failed to Initialize_Prototype");
	}

	return pInstance;
}

shared_ptr<CGameObject> CHaruka::Clone(any)
{
	shared_ptr<CHaruka> pInstance = make_private_shared_copy(CHaruka, *this);

	if (FAILED(pInstance->Initialize()))
	{
		MSG_RETURN(nullptr, "CHaruka::Clone", "Failed to Initialize");
	}
