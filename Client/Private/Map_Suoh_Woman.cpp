...
CMap_Suoh_Woman::CMap_Suoh_Woman(ComPtr<ID3D11Device> _pDevice, ComPtr<ID3D11DeviceContext> _pContext)
	: CGameObject(_pDevice, _pContext)
{
}

CMap_Suoh_Woman::CMap_Suoh_Woman(const CMap_Suoh_Woman& _rhs)
	:CGameObject(_rhs)
{
}

HRESULT CMap_Suoh_Woman::Initialize_Prototype(_uint iIndex)
{
	...
	m_umapComponentArg[COMPONENT::MODEL] = make_pair(PROTOTYPE_COMPONENT_MODEL_SUOH_NPC_WOMAN, g_aNull);

	return S_OK;
}

HRESULT CMap_Suoh_Woman::Initialize(any pArg)
{
	if (FAILED(__super::Initialize()))
	{
		MSG_RETURN(E_FAIL, "CMap_Suoh_Woman::Initialize", "Failed to CGameObject::Initialize");
	}
	...
	//m_eWomanState = WOMANSTATE::STATE_IDLE;
	m_ePointPos = POINTPOS::POS_FIRST;

	m_WomanPos.CreatePos = m_Translation;
...
#endif
void CMap_Suoh_Woman::Tick(_float fTimeDelta)
{
#if ACTIVATE_IMGUI
	bCheckOnce = true;
#endif
	...
	if (m_bSceneMapTool)
	{
		...
		{
			m_eWomanState = WOMANSTATE::STATE_WALK;
		}
		...
		{
			m_eWomanState = WOMANSTATE::STATE_TALK;
		}
	...
	//if (ImGui::CollapsingHeader("Woman Material"))
	//{
	...
	//			if (ImGui::SliderFloat4(("Diffuse##Woman" + std::to_string(i)).c_str(), &tShaderDesc.tMaterialDesc.vDiffuse.x, 0.f, 1.f))
	//				bDirtyFlag = true;
	//			if (ImGui::SliderFloat4(("Ambient##Woman" + std::to_string(i)).c_str(), &tShaderDesc.tMaterialDesc.vAmbient.x, 0.f, 1.f))
	//				bDirtyFlag = true;
	//			if (ImGui::SliderFloat4(("Specular##Woman" + std::to_string(i)).c_str(), &tShaderDesc.tMaterialDesc.vSpecular.x, 0.f, 1.f))
	//				bDirtyFlag = true;
	//			if (ImGui::SliderFloat4(("Emissive##Woman" + std::to_string(i)).c_str(), &tShaderDesc.tMaterialDesc.vEmissive.x, 0.f, 1.f))
	//				bDirtyFlag = true;
	//			if (ImGui::SliderFloat(("Shininess##Woman" + std::to_string(i)).c_str(), &tShaderDesc.tMaterialDesc.fShininess, 0.f, 1.f))
	//				bDirtyFlag = true;
...
void CMap_Suoh_Woman::Late_Tick(_float fTimeDelta)
{
#if ACTIVATE_IMGUI
	...
	if (bCheckOnce)
		if (CImGui_Manager::Get_Instance()->Is_Enable())
		{
			bCheckOnce = false;
			ImGui::Begin("Please");
			ImGui::Checkbox("CMap_Suoh_Woman", &bRender);
			ImGui::End();
...
HRESULT CMap_Suoh_Woman::Render()
{
...
HRESULT CMap_Suoh_Woman::Render_ShadowDepth(shared_ptr<CLight> _pShadow)
{
	if (FAILED(__super::Render_ShadowDepth(_pShadow)))
	{
		MSG_RETURN(E_FAIL, "CMap_Suoh_Woman::Render_ShadowDepth", "Failed to CGameObject::Render_ShadowDepth");
	}

	if (FAILED(m_pModel->Render_ShadowDepth(shared_from_gameobject(), m_pShader, 2)))
	{
		MSG_RETURN(E_FAIL, "CMap_Suoh_Woman::Render_ShadowDepth", "Failed to Render_ShadowDepth");
	}

	return S_OK;
}

HRESULT CMap_Suoh_Woman::Render_OutLine()
{
	return S_OK;
}

HRESULT CMap_Suoh_Woman::Ready_Components()
{
...
HRESULT CMap_Suoh_Woman::Ready_Parts()
{
	return S_OK;
}

void CMap_Suoh_Woman::Using_Map_Tool()
{
...
void CMap_Suoh_Woman::State_Animation(_float fTimeDelta)
{
	Go_Point();

	switch (m_eWomanState)
	{
	case WOMANSTATE::STATE_IDLE:
		m_pModel->Set_Animation(2, 1.f, false, 0.1f, true);
		break;
	case WOMANSTATE::STATE_WALK:
		...
		break;
	case WOMANSTATE::STATE_RUN:
		m_pModel->Set_Animation(6, 3.f, false, 0.1f, true);
		m_pTransform->Set_State(TRANSFORM::LOOK, m_Dir);
		m_pTransform->Translate(m_pTransform->Get_State(TRANSFORM::LOOK) * fTimeDelta * m_fSpeed);
		break;
	case WOMANSTATE::STATE_TALK:
		m_pModel->Set_Animation(44, 3.f, false, 0.1f, true);
		break;
	default:
		break;
	}
...
void CMap_Suoh_Woman::Go_Point()
{
	...
	if (m_eRoundTrip == ROUNDTRIP::ROUNDTRIP_SECOND)
	{
		...
		case POINTPOS::POS_FIRST:
			m_Dir = m_WomanPos.Pos[0] - m_WomanPos.CreatePos;
			m_Dir.y = 0.f;
			tmpDistance.x = abs(CurrentPos.x - m_WomanPos.Pos[0].x);
			tmpDistance.z = abs(CurrentPos.z - m_WomanPos.Pos[0].z);
		...
		case POINTPOS::POS_SECOND:
			m_Dir = m_WomanPos.CreatePos - m_WomanPos.Pos[0];
			m_Dir.y = 0.f;
			tmpDistance.x = abs(CurrentPos.x - m_WomanPos.CreatePos.x);
			tmpDistance.z = abs(CurrentPos.z - m_WomanPos.CreatePos.z);
			if (tmpDistance.x <= 1.f && tmpDistance.z <= 1.f)
			{
				m_ePointPos = POINTPOS::POS_FIRST;
			}
	...
	else if (m_eRoundTrip == ROUNDTRIP::ROUNDTRIP_SIXTH)
	{
		...
		case POINTPOS::POS_FIRST:
			...
			break;
		case POINTPOS::POS_SECOND:
			m_Dir = m_WomanPos.Pos[1] - m_WomanPos.Pos[0];
			tmpDistance.x = abs(CurrentPos.x - m_WomanPos.Pos[1].x);
			tmpDistance.z = abs(CurrentPos.z - m_WomanPos.Pos[1].z);
			if (tmpDistance.x <= 1.f && tmpDistance.z <= 1.f)
			{
		...
		case POINTPOS::POS_THIRD:
			m_Dir = m_WomanPos.Pos[2] - m_WomanPos.Pos[1];
			tmpDistance.x = abs(CurrentPos.x - m_WomanPos.Pos[2].x);
			tmpDistance.z = abs(CurrentPos.z - m_WomanPos.Pos[2].z);
			if (tmpDistance.x <= 1.f && tmpDistance.z <= 1.f)
			{
		...
		case POINTPOS::POS_FORTH:
			m_Dir = m_WomanPos.Pos[1] - m_WomanPos.Pos[2];
			tmpDistance.x = abs(CurrentPos.x - m_WomanPos.Pos[1].x);
			tmpDistance.z = abs(CurrentPos.z - m_WomanPos.Pos[1].z);
			if (tmpDistance.x <= 1.f && tmpDistance.z <= 1.f)
			{
		...
		case POINTPOS::POS_FIFTH:
			m_Dir = m_WomanPos.Pos[0] - m_WomanPos.Pos[1];
			tmpDistance.x = abs(CurrentPos.x - m_WomanPos.Pos[0].x);
			tmpDistance.z = abs(CurrentPos.z - m_WomanPos.Pos[0].z);
			if (tmpDistance.x <= 1.f && tmpDistance.z <= 1.f)
			{
...
void CMap_Suoh_Woman::Input_Pos()
{
	if (m_strNameNum == TEXT("Suoh_NPC_Woman_number0"))
	{
		m_WomanPos.Pos[0] = _float3(-14.8f, 0.f, 256.f);
		m_eRoundTrip = ROUNDTRIP::ROUNDTRIP_SECOND;
		m_eWomanState = WOMANSTATE::STATE_WALK;
		m_fSpeed = 0.92f;
	}
	if (m_strNameNum == TEXT("Suoh_NPC_Woman_number2") ||
		m_strNameNum == TEXT("Suoh_NPC_Woman_number3"))
	{
		m_eWomanState = WOMANSTATE::STATE_IDLE;
	}
	if (m_strNameNum == TEXT("Suoh_NPC_Woman_number1")||
		m_strNameNum == TEXT("Suoh_NPC_Woman_number4") ||
		m_strNameNum == TEXT("Suoh_NPC_Woman_number5"))
	{
		m_eWomanState = WOMANSTATE::STATE_TALK;
	}
...
shared_ptr<CMap_Suoh_Woman> CMap_Suoh_Woman::Create(ComPtr<ID3D11Device> _pDevice, ComPtr<ID3D11DeviceContext> _pContext, _int iIndex)
{
	shared_ptr<CMap_Suoh_Woman> pInstance = make_private_shared(CMap_Suoh_Woman, _pDevice, _pContext);

	if (FAILED(pInstance->Initialize_Prototype(iIndex)))
	{
		MSG_RETURN(nullptr, "CMap_Suoh_Woman::Create", "Failed to Initialize_Prototype");
	}

	return pInstance;
}

shared_ptr<CGameObject> CMap_Suoh_Woman::Clone(any pArg)
{
	shared_ptr<CMap_Suoh_Woman> pInstance = make_private_shared_copy(CMap_Suoh_Woman, *this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_RETURN(nullptr, "CMap_Suoh_Woman::Clone", "Failed to Initialize");
	}
