...
#define MOVIE_MAX       206

CUI_Indicator_ToolTip_Mizuha_SAS_Acceleration::CUI_Indicator_ToolTip_Mizuha_SAS_Acceleration(ComPtr<ID3D11Device> _pDevice, ComPtr<ID3D11DeviceContext>_pContext)
    : CGameObject(_pDevice, _pContext)
{
}

HRESULT CUI_Indicator_ToolTip_Mizuha_SAS_Acceleration::Initialize_Prototype()
{
...
HRESULT CUI_Indicator_ToolTip_Mizuha_SAS_Acceleration::Initialize(any _aDesc)
{
    if (FAILED(__super::Initialize()))
    {
        MSG_RETURN(E_FAIL, "CUI_Indicator_ToolTip_Mizuha_SAS_Acceleration::Initialize", "Failed to CGameObject::Initialize");
    }
    ...
    {
        MSG_RETURN(E_FAIL, "CUI_Indicator_ToolTip_Mizuha_SAS_Acceleration::Initialize", "Failed to Get_Component: CVIBuffer_Rect");
    }

    return S_OK;
}

void CUI_Indicator_ToolTip_Mizuha_SAS_Acceleration::Tick(_float fTimeDelta)
{
    __super::Tick(fTimeDelta);
}

void CUI_Indicator_ToolTip_Mizuha_SAS_Acceleration::Late_Tick(_float fTimeDelta)
{
...
HRESULT CUI_Indicator_ToolTip_Mizuha_SAS_Acceleration::Render()
{
    if (true == m_bMovieStart)
    {
        ...
        CGameInstance::Get_Instance()->Render_Font(TEXT("Font_ScarletStrings"), m_strExplanation5,  _float2(603.f, 680.f),  XMVectorSet(m_fSeperateLineTime, m_fSeperateLineTime, m_fSeperateLineTime, m_fMovieShaderTime), 0.5f);
...
HRESULT CUI_Indicator_ToolTip_Mizuha_SAS_Acceleration::Ready_Components()
{
...
void CUI_Indicator_ToolTip_Mizuha_SAS_Acceleration::Initialize_UI()
{
//=========================================================================== _float
...
//=========================================================================== Texture
    ...
    m_pTexture_Movie                    = CTexture::Create(m_pDevice, m_pContext, TEXT("Bin/Resources/Texture/UI/Indicator/Mizuha_Tips/SAS_Acceleration/T_ui_TutoTips_027_01_common_m_MV_Movie_40534656.mkv_%d.png"), MOVIE_MAX, true);
...
//=========================================================================== Explanation
    m_strTitle          = L"SAS 초고속";
    m_strExplanation1   = L"초고속 뇌능력을 빌려 아주 빠른 속도로 움직일 수 있게 돼";
    m_strExplanation2   = L"마치 주위에 흐르는 시간이 느려진 것처럼 느껴진다.";
    m_strExplanation3   = L"재빠른 적과 대치하거나 단숨에 공격을 가하고 싶을 때";
    m_strExplanation4   = L"발동할 것을 추천한다.";
    m_strExplanation5   = L"아라시와의 유대가 깊어질수록 효과가 늘어난다.";
    m_strEnter          = L"닫기";

}

void CUI_Indicator_ToolTip_Mizuha_SAS_Acceleration::Tick_UI(_float fTimeDelta)
{
...
void CUI_Indicator_ToolTip_Mizuha_SAS_Acceleration::Movie_System(_float fTimeDelta)
{
...
void CUI_Indicator_ToolTip_Mizuha_SAS_Acceleration::Sound(_float fTimeDelta)
{
    if (true == m_bMovieStart && false == m_bToolTip_End)
    {
        if (0 == m_iSoundIndex)
...
void CUI_Indicator_ToolTip_Mizuha_SAS_Acceleration::Movie_Start()
{
...
void CUI_Indicator_ToolTip_Mizuha_SAS_Acceleration::Movie_End(_float fTimeDelta)
{
...
shared_ptr<CUI_Indicator_ToolTip_Mizuha_SAS_Acceleration> CUI_Indicator_ToolTip_Mizuha_SAS_Acceleration::Create(ComPtr<ID3D11Device> _pDevice, ComPtr<ID3D11DeviceContext> _pContext)
{
    shared_ptr<CUI_Indicator_ToolTip_Mizuha_SAS_Acceleration> pInstance = make_private_shared(CUI_Indicator_ToolTip_Mizuha_SAS_Acceleration, _pDevice, _pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_RETURN(nullptr, "CUI_Indicator_ToolTip_Mizuha_SAS_Acceleration::Create", "Failed to Initialize_Prototype");
    }

    return pInstance;
}

shared_ptr<CGameObject> CUI_Indicator_ToolTip_Mizuha_SAS_Acceleration::Clone(any aDesc)
{
    shared_ptr<CUI_Indicator_ToolTip_Mizuha_SAS_Acceleration> pInstance = make_private_shared_copy(CUI_Indicator_ToolTip_Mizuha_SAS_Acceleration, *this);
