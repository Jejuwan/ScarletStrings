...
#define MOVIE_MAX       341

CUI_Indicator_ToolTip_Mizuha_SAS_Stealth::CUI_Indicator_ToolTip_Mizuha_SAS_Stealth(ComPtr<ID3D11Device> _pDevice, ComPtr<ID3D11DeviceContext> _pContext)
	: CGameObject(_pDevice, _pContext)
{
}

HRESULT CUI_Indicator_ToolTip_Mizuha_SAS_Stealth::Initialize_Prototype()
{
...
HRESULT CUI_Indicator_ToolTip_Mizuha_SAS_Stealth::Initialize(any _aDesc)
{
...
void CUI_Indicator_ToolTip_Mizuha_SAS_Stealth::Tick(_float fTimeDelta)
{
    __super::Tick(fTimeDelta);
}

void CUI_Indicator_ToolTip_Mizuha_SAS_Stealth::Late_Tick(_float fTimeDelta)
{
...
HRESULT CUI_Indicator_ToolTip_Mizuha_SAS_Stealth::Render()
{
    if (true == m_bMovieStart)
    {
        ...
        CGameInstance::Get_Instance()->Render_Font(TEXT("Font_ScarletStrings"), m_strExplanation1_1,    _float2(953.f, 500.f), XMVectorSet(255.f * m_fSeperateLineTime / 255.f, 180.f * m_fSeperateLineTime / 255.f, 180.f * m_fSeperateLineTime / 255.f, m_fMovieShaderTime), 0.5f);
        CGameInstance::Get_Instance()->Render_Font(TEXT("Font_ScarletStrings"), m_strExplanation2,      _float2(603.f, 540.f), XMVectorSet(255.f * m_fSeperateLineTime / 255.f, 180.f * m_fSeperateLineTime / 255.f, 180.f * m_fSeperateLineTime / 255.f, m_fMovieShaderTime), 0.5f);
        CGameInstance::Get_Instance()->Render_Font(TEXT("Font_ScarletStrings"), m_strExplanation2_1,    _float2(783.f, 540.f), XMVectorSet(m_fSeperateLineTime, m_fSeperateLineTime, m_fSeperateLineTime, m_fMovieShaderTime), 0.5f);
        ...
        CGameInstance::Get_Instance()->Render_Font(TEXT("Font_ScarletStrings"), m_strExplanation4_1,    _float2(653.f, 620.f), XMVectorSet(255.f * m_fSeperateLineTime / 255.f, 180.f * m_fSeperateLineTime / 255.f, 180.f * m_fSeperateLineTime / 255.f, m_fMovieShaderTime), 0.5f);
        CGameInstance::Get_Instance()->Render_Font(TEXT("Font_ScarletStrings"), m_strExplanation4_2,    _float2(943.f, 620.f), XMVectorSet(m_fSeperateLineTime, m_fSeperateLineTime, m_fSeperateLineTime, m_fMovieShaderTime), 0.5f);
        CGameInstance::Get_Instance()->Render_Font(TEXT("Font_ScarletStrings"), m_strExplanation5,      _float2(603.f, 660.f), XMVectorSet(m_fSeperateLineTime, m_fSeperateLineTime, m_fSeperateLineTime, m_fMovieShaderTime), 0.5f);
...
HRESULT CUI_Indicator_ToolTip_Mizuha_SAS_Stealth::Ready_Components()
{
    if (FAILED(__super::Ready_Components()))
    {
        MSG_RETURN(E_FAIL, "CUI_Indicator_ToolTip_Mizuha_SAS_Electric::Ready_Components", "Failed to CGameObject::Ready_Components");
    }
    m_pShader = Get_Component<CShader>(COMPONENT::SHADER);
    if (nullptr == m_pShader)
    {
        MSG_RETURN(E_FAIL, "CUI_Indicator_ToolTip_Mizuha_SAS_Electric::Ready_Components", "Failed to Get_Component: SHADER");
    }

    return S_OK;
}

void CUI_Indicator_ToolTip_Mizuha_SAS_Stealth::Initialize_UI()
{
//=========================================================================== _float
...
//=========================================================================== Texture
    ...
    m_pTexture_Movie                    = CTexture::Create(m_pDevice, m_pContext, TEXT("Bin/Resources/Texture/UI/Indicator/Mizuha_Tips/SAS_Stealth/T_ui_TutoTips_029_01_common_f_MV_Movie_40534656.mkv_%d.png"), MOVIE_MAX, true);
...
//=========================================================================== _float3
    ...
    m_vScale_Key_Z                      = _float3(30.f, 30.f, 1.f);
...
//=========================================================================== _float4
    ...
    m_vPosition_Key_Z                   = _float4(-40.f, -90.f, 0.f, 1.f);
...
//=========================================================================== Explanation
    m_strTitle          = L"SAS 투명화";
    m_strExplanation1   = L"투명화 뇌능력을 빌려 모습을 숨김으로써";
    m_strExplanation1_1 = L"적에게 존재가";
    m_strExplanation2   = L"감지되지 않게 된다.";
    m_strExplanation2_1 = L"모습을 숨기고 적의 약점이 있는";
    m_strExplanation3   = L"위치로 돌아 들어가 공격하는 전법을 취할 수 있다.";
    m_strExplanation4   = L"또한,";
    m_strExplanation4_1 = L"적에게 들키지 않은 상태에서";
    m_strExplanation4_2 = L"의 특수공격을 맞히면 ";
    m_strExplanation5   = L"데미지가 상승하는 <크리티컬 히트> 가 반드시 발생한다.";

    m_strEnter          = L"닫기";
}

void CUI_Indicator_ToolTip_Mizuha_SAS_Stealth::Tick_UI(_float fTimeDelta)
{
...
void CUI_Indicator_ToolTip_Mizuha_SAS_Stealth::Movie_System(_float fTimeDelta)
{
...
void CUI_Indicator_ToolTip_Mizuha_SAS_Stealth::Sound(_float fTimeDelta)
{
    if (true == m_bMovieStart && false == m_bToolTip_End)
    {
        if (0 == m_iSoundIndex)
...
void CUI_Indicator_ToolTip_Mizuha_SAS_Stealth::Movie_Start()
{
...
void CUI_Indicator_ToolTip_Mizuha_SAS_Stealth::Movie_End(_float fTimeDelta)
{
...
shared_ptr<CUI_Indicator_ToolTip_Mizuha_SAS_Stealth> CUI_Indicator_ToolTip_Mizuha_SAS_Stealth::Create(ComPtr<ID3D11Device> _pDevice, ComPtr<ID3D11DeviceContext> _pContext)
{
    shared_ptr<CUI_Indicator_ToolTip_Mizuha_SAS_Stealth> pInstance = make_private_shared(CUI_Indicator_ToolTip_Mizuha_SAS_Stealth, _pDevice, _pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_RETURN(nullptr, "CUI_Indicator_ToolTip_Mizuha_SAS_Stealth::Create", "Failed to Initialize_Prototype");
    }

    return pInstance;
}

shared_ptr<CGameObject> CUI_Indicator_ToolTip_Mizuha_SAS_Stealth::Clone(any aDesc)
{
    shared_ptr<CUI_Indicator_ToolTip_Mizuha_SAS_Stealth> pInstance = make_private_shared_copy(CUI_Indicator_ToolTip_Mizuha_SAS_Stealth, *this);
