...
class CMap_Suoh_Man : public CGameObject
{
public:
	enum class MANSTATE { STATE_IDLE, STATE_WALK, STATE_RUN, STATE_TALK, STATE_END};
...
private:
	explicit CMap_Suoh_Man(ComPtr<ID3D11Device>, ComPtr<ID3D11DeviceContext>);
	explicit CMap_Suoh_Man(const CMap_Suoh_Man&);
	virtual ~CMap_Suoh_Man() DEFAULT;
...
private:
	...
	_float m_fSpeed = { 0.f };
public:
	static shared_ptr<CMap_Suoh_Man>		Create(ComPtr<ID3D11Device>, ComPtr<ID3D11DeviceContext>, _int iIndex);
