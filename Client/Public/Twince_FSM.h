...
private:
    ...
    //Wait
private:
    void BeginState_Wait();
    void ExecuteState_Wait(_float fTimeDelta);
    void EndState_Wait();
    //Hide
private:
    void BeginState_Hide();
    void ExecuteState_Hide(_float fTimeDelta);
    void EndState_Hide();
    //Down
private:
    void BeginState_Down();
    void ExecuteState_Down(_float fTimeDelta);
    void EndState_Down();
    //Beam
private:
    void BeginState_Beam();
    void ExecuteState_Beam(_float fTimeDelta);
    void EndState_Beam();
    //Beam_OneWay
private:
    void BeginState_Beam_OneWay();
    void ExecuteState_Beam_OneWay(_float fTimeDelta);
    void EndState_Beam_OneWay();
    //OnHit_Front
private:
...
private :
    weak_ptr<CMonster_Twince>                               m_pOwner;

private :
    shared_ptr<CStateTemplate<CFSM>>			            m_StateInitial;
    shared_ptr<CStateTemplate<CTwince_FSM>>			        m_pState_Idle;
    shared_ptr<CStateTemplate<CTwince_FSM>>			        m_pState_Wait;
    shared_ptr<CStateTemplate<CTwince_FSM>>			        m_pState_Hide;
    shared_ptr<CStateTemplate<CTwince_FSM>>			        m_pState_Down;
    shared_ptr<CStateTemplate<CTwince_FSM>>			        m_pState_Beam;
    shared_ptr<CStateTemplate<CTwince_FSM>>			        m_pState_Beam_OneWay;
    shared_ptr<CStateTemplate<CTwince_FSM>>			        m_pState_OnHit_Front;
    shared_ptr<CStateTemplate<CTwince_FSM>>			        m_pState_OnHit_Back;
    shared_ptr<CStateTemplate<CTwince_FSM>>			        m_pState_OnHit_Psy_Front;
    shared_ptr<CStateTemplate<CTwince_FSM>>			        m_pState_OnHit_Psy_Back;
    shared_ptr<CStateTemplate<CTwince_FSM>>			        m_pState_OnHit_Psy_Left;
    shared_ptr<CStateTemplate<CTwince_FSM>>			        m_pState_OnHit_Psy_Right;
    shared_ptr<CStateTemplate<CTwince_FSM>>			        m_pState_AirBone;
    shared_ptr<CStateTemplate<CTwince_FSM>>			        m_pState_AirBone_Falling;
    shared_ptr<CStateTemplate<CTwince_FSM>>			        m_pState_Dead;
    shared_ptr<CStateTemplate<CTwince_FSM>>			        m_pState_BCChance;
    shared_ptr<CStateTemplate<CTwince_FSM>>			        m_pState_FinishMotion;
...
private :
    ...
    shared_ptr<CTransform>                                 m_pEffect;
...
private:
     ...
     shared_ptr<class CUI_Indicator_Monster_MissPlayer>     m_pUI_MissPlayer;
     shared_ptr<class CUI_Indicator_InstKill>               m_pUI_InstKIll;


public:
    static shared_ptr<CTwince_FSM> Create(ComPtr<ID3D11Device> pDevice, ComPtr<ID3D11DeviceContext> pContext, shared_ptr<CGameObject> Owner, ENTITYDESC* pEntityDesczz, ACTORDESC* pActorDesc);
};
