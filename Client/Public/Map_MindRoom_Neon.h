...
class CMap_MindRoom_Neon : public CGameObject
{
private:
	explicit CMap_MindRoom_Neon(ComPtr<ID3D11Device>, ComPtr<ID3D11DeviceContext>);
	explicit CMap_MindRoom_Neon(const CMap_MindRoom_Neon&);
	virtual ~CMap_MindRoom_Neon() DEFAULT;
...
public:
	void								Set_NeonColor(const _float4& vNeonColor);

private:
	virtual HRESULT						Ready_Components() override;
	void	Using_Map_Tool();
	_bool	Is_Twist(wstring strTwist);
...
private:
	...
	shared_ptr<CTexture>				m_pNeonCrashTexture;
...
private:
	...
	INSTMODELMESHDESC					m_tInstNeonMeshDesc;
...
public:
	static shared_ptr<CMap_MindRoom_Neon>		Create(ComPtr<ID3D11Device>, ComPtr<ID3D11DeviceContext>, _int iIndex);
