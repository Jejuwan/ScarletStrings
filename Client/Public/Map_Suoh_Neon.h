...
class CMap_Suoh_Neon : public CGameObject
{
private:
	explicit CMap_Suoh_Neon(ComPtr<ID3D11Device>, ComPtr<ID3D11DeviceContext>);
	explicit CMap_Suoh_Neon(const CMap_Suoh_Neon&);
	virtual ~CMap_Suoh_Neon() DEFAULT;
...
private:
	...
	HRESULT	Neon_Render(wstring MeshName);
	void	Mesh_Index(wstring MeshName);

public:
	static shared_ptr<CMap_Suoh_Neon>		Create(ComPtr<ID3D11Device>, ComPtr<ID3D11DeviceContext>, _int iIndex);
